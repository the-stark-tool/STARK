...
public class TwoLanesTwoCarsPhantom {
    ...
    public TwoLanesTwoCarsPhantom() throws IOException {
        try {
            ...
            int PERTURBATION_SIZE = 50; // default to 100, manually set to 50 bcs it runs for too long
            int EXTRA_SIZE = 1; // set to 1 to generate a single clean trajectory for the visualizer
            ...
            if (SCENARIO == 1) {
                ...
                Util.writeToCSV("./phantom_extra_trajectory_p.csv", phantom_extra_trajectory_p);
            } else if (SCENARIO == 2) {
                ...
                Util.writeToCSV("./phantom_extra_trajectory_p_scen2.csv", phantom_extra_trajectory_p);
            } else {
                ...
                Util.writeToCSV("./phantom_extra_trajectory_p_scen3.csv", phantom_extra_trajectory_p);
            }
            ...
            DistanceExpression max_crash_phantom = new MaxIntervalDistanceExpression(crash, 0, H);
            RobustnessFormula phi_phantom_attack = new AlwaysRobustnessFormula(
                    new AtomicRobustnessFormula(getPhantomAttackPerturbation(),
                            max_crash_phantom,
                            RelationOperator.LESS_OR_EQUAL_THAN,
                            eta),
                    0, 100);
            ...
            for (int i = 0; i < 10; i++) {
                Boolean value = new BooleanSemanticsVisitor().eval(phi_phantom_attack)
                        .eval(PERTURBATION_SIZE, i, sequence);
                System.out.println("Phi_phantom_attack evaluation at step " + i + ": " + value);
                if (value) {
            ...
            if (SCENARIO == 1) {
                Util.writeToCSV("./three_val_phantom_attack.csv", val_phantom_attack);
            } else if (SCENARIO == 2) {
                Util.writeToCSV("./three_val_phantom_attack_scen2.csv", val_phantom_attack);
            } else {
                Util.writeToCSV("./three_val_phantom_attack_scen3.csv", val_phantom_attack);
            }
    ...
    private static Perturbation getPhantomAttackPerturbation() {
        // Phase A: Lane transition spoofing (repeats for 50 steps)
        // Phase B: Close-proximity spoofing (runs for 250 steps)
        return new SequentialPerturbation(
                new IterativePerturbation(50,
                        new AtomicPerturbation(0, TwoLanesTwoCarsPhantom::phantomDuringTransition)),
                new IterativePerturbation(250,
    ...
    private static DataState phantomDuringTransition(RandomGenerator rg, DataState state) {
        ...
        if (my_current_y >= 4) {
            ...
            double phx = state.get(my_x) + 100.0;
    ...
    private static DataState phantomCloseProximity(RandomGenerator rg, DataState state) {
        ...
        if (does_attacker_love_M) {
            ...
            trigger_dist = Math.max(80.0, stopping_dist + 20.0);
        } else {
            trigger_dist = 50.0;
        }
