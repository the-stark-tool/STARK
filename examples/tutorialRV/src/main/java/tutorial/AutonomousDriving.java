    ...
    public static void main(String[] args) throws IOException{
        try {
            ...
            */
            ...
            <code>get_reckless_driver()</code> defined later.
            The two evolution sequences are generated by method <code>printLData</code>, which prints out, for each of
            the <code>H</code> steps, the average value assumed by the expressions on data states in list <code>F</code>.
            This allows us to see an evidence of the result of the simulations.
            */
...
/*
            The following lines of code first define two atomic distances between evolution sequences, named
            <code>crash</code> and <code>crash_speed</code>.
            Technically, <code>crash</code> and <code>crash_speed</code> are atomic distances in the sense that
            they are instances of class <code>AtomicDistanceExpression</code>, which consists in a data state expression,
            mapping a data state to a number, or rank, and a binary operator.
            Given two configurations, the data state expression allow us to get two numbers from them, to which
            the binary operator is applied. Intuitively, the result quantifies some difference between the two
            configurations.
            In case of <code>crash</code> the number is the value of variable <code>crash</code>, which is zero in
            the nominal system, and the binary operator is the absolute value of the difference. Therefore, the
            perturbed configuration and the nominal one differ precisely in case of crash.
            In case of <code>crash_speed</code> the number is the severity of impact, which is zero in
            the nominal system, and the binary operator is the absolute value of the difference. Therefore, the
            perturbed configuration and the nominal one differ precisely in case of crash and the difference is
            proportional to the severity of impact.
            The distance between configurations will be lifted to two sample sets of configurations, those obtained from
            <code>sequence</code> and <code>perturbed_sequence</code> at the same step.

            Then, these distances are evaluated, time-point by time-point, over
            evolution sequence <code>sequence</code> and its perturbed version <code>perturbed_sequence</code> defined above.
            ...
            */
            ...
            /*
            Below we define two instantes of <code>MaxIntervalDistanceExpression</code>, each consisting
            in an argument <code>DistanceExpression</code> and an interval.
            Intuitively, the <code>MaxIntervalDistanceExpression</code> evaluates the argument expression
            in all points in the interval and returns the maximal value
            */
            ...
            /*
            Now we define two robustness formulas, named <code>phi_crash</code> and <code>phi_si</code>.
            In particular these are two atomic formulas, namely instances of <code>AtomicRobustnessFormula</code>.
            These formula will be evaluated on the evolution sequence <code>sequence</code> and express that the
            distance, expressed by expression distance <code>max_crash</code> and <code>max_si</code>, respectively,
            between that evolution sequence and the evolution sequence obtained from it by applying the perturbation
            returned by method <code>get_reckless_driver()</code>, is below a given threshold <code>eta</code>.
            Then, we define the <code>ConjunctionRobustnessFormula</code> named <code>phi_combined</code> which,
            intuitively represents the conjunction between <code>phi_crash</code> and <code>phi_si</code>, and
            the <code>AlwaysRobustnessFormula</code> named <code>phi_SAF</code> which, intuitively, evaluates
            <code>phi_combined</code> in all points in an interval and returns the worst value.
             */
