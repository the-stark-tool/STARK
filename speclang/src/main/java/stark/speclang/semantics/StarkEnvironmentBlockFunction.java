 ...
 */
public class StarkEnvironmentBlockFunction extends StarkAbstractEnvironmentFunction {

    private final List<StarkEnvironmentUpdateFunction> commands;
     ...
     */
    public StarkEnvironmentBlockFunction(StarkVariableAllocation allocation, List<StarkEnvironmentUpdateFunction> commands) {
        super(allocation);
    ...
    public List<DataStateUpdate> apply(RandomGenerator randomGenerator, StarkStore starkStore) {
        return commands.stream().flatMap(c -> c.apply(randomGenerator, starkStore).stream()).toList();
    }
}
